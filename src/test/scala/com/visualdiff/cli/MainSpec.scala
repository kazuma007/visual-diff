...
class MainSpec extends AnyFunSpec {
  ...
  describe("Main.run") {
    it("executes successfully and generates report") {
      ...
      val config = Config(oldPdf = pdf, newPdf = pdf, outputDir = dir.resolve("out"))
    ...
    it("fails validation if PDF does not exist") {
      ...
      val config = Config(
        oldPdf = dir.resolve("non_existent.pdf"),
        newPdf = dir.resolve("non_existent_2.pdf"),
    ...
    it("respects fail-on-diff flag") {
      ...
      val config = Config(oldPdf = pdf, newPdf = pdf, failOnDiff = true, outputDir = dir)
    ...
    it("creates output directory if it doesn't exist") {
      ...
      val config = Config(oldPdf = pdf, newPdf = pdf, outputDir = outputDir)
      val result = Main.run(config)
    ...
    it("handles custom threshold settings") {
      ...
      val config = Config(
        oldPdf = pdf, newPdf = pdf, outputDir = dir.resolve("out"), thresholdPixel = 0.05, thresholdLayout = 10.0,
      )
    ...
    it("fails when old PDF path is invalid") {
      ...
      val config = Config(
        oldPdf = dir.resolve("invalid.pdf"),
        newPdf = validPdf,
        outputDir = dir,
      )
    ...
    it("fails when new PDF path is invalid") {
      ...
      val config = Config(
        oldPdf = validPdf,
        newPdf = dir.resolve("invalid.pdf"),
        outputDir = dir,
      )
    ...
    it("handles multi-page PDF comparison") {
      ...
      val config = Config(oldPdf = pdf1, newPdf = pdf2, outputDir = dir.resolve("out"))
