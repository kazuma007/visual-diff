...
`visual-diff` is a JVM/Scala-based command-line tool that compares two PDF files and generates a comprehensive report on their differences.
...
-   **Text Diff**: Extracts all text content with coordinates to identify added or removed text.
-   **Layout Diff**: Detects when text or elements have shifted beyond a configurable threshold.
-   **Font Diff**: Identifies font substitutions, missing fonts, and embedding status changes that can affect document appearance.
...
Execute the JAR, providing the paths to the two PDF files you want to compare.
...
|----------------------------------| ----- |------------| ------------------------------------------------------------------------- |
| `--old-pdf <oldPdf>`             |       | (required) | Path to the old/baseline PDF file.                                        |
| `--new-pdf <newPdf>`             |       | (required) | Path to the new/modified PDF file.                                        |
...
| `--thresholdLayout <pixels>`     |       | `0.0`      | Layout shift threshold in pixels. Changes above this are flagged.         |
...
| `--dpi <number>`                 |       | `150`      | DPI (Dots Per Inch) for rendering PDFs to images. Higher = more detail.   |
...
-   **`report.html`**: Visual HTML report with summary statistics and highlighted differences
    -   Font differences shown first with scrollable section (if many)
...
-   **`diff.json`**: Structured JSON containing all detected differences with metadata:
    ...
    -   `textDiffs`: Added/removed text with bounding boxes
    -   `layoutDiffs`: Element displacement measurements
    -   `fontDiffs`: Font substitution details
    -   `suppressedDiffs`: Metadata about what was suppressed and why (preserves all data)
-   **`old_p*.png`**: PNG images of pages from the old PDF
-   **`new_p*.png`**: PNG images of pages from the new PDF  
